<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
		<!-- S : HEADER -->			
		<div id="header">	
			<div class="gnb-area">				
				<a href="javascript:guestHome();"><img src="resources/img/logo.png"></a>
								
				<ul class="menu">				
					<li><a href="#">Notice</a>
						<ul class="dep2">
							<li><a href="guestboardlist?category=공지사항">Notice</a></li>						
						</ul></li>
					<li><a href="#">Rooms & Suites</a>
						<ul class="dep2">
							<li><a href="room_infoList">ViewAll</a></li>
							<li><a href="room_infoTitle?rname=DELUXE">Deluxe</a></li>
							<li><a href="room_infoTitle?rname=EXCUTIVE">Executive</a></li>
							<li><a href="room_infoTitle?rname=SUITE">Suites</a></li>
						</ul></li>	
					<li><a href="#">Restaurant</a>
						<ul class="dep2">
							<li><a href="">Dining</a></li>
							<li><a href="">Bar</a></li>						
						</ul></li>						
					<li><a href="#">Facilities</a>
						<ul class="dep2">
							<li><a href="">Gymnasium</a></li>
							<li><a href="">Pool</a></li>
							<li><a href="">Golf</a></li>
							<li><a href="">Convenient Facilities</a></li>
						</ul></li>				
					<li><a href="#">Board</a>
						<ul class="dep2">
							<li><a href="guestboardlist?category=QnA">QnA</a></li>
							<li><a href="guestboardlist?category=환불신청게시판">Refund Board</a></li>
							<li><a href="guestboardlist?category=자유게시판">Free Board</a></li>
						</ul></li>											
				</ul>
				<a href="" class="btn-reservation">Reservations</a>
			</div>
		</div>	
		<!-- E : HEADER -->	