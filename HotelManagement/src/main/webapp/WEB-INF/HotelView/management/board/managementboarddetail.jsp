				...
				$("#memo1").click(
						function() {
							window.open("loginform", "",
									"width=350 height=550 scrollbars=yes");
						});
	...
	<div id="main_content">
		...
		<form action="boardReply" method="post">
			...
			<input type="hidden" name="groups" id="groups"
				value="${board.groups}"> <input type="hidden" name="levels"
				id="levels" value="${board.levels}"> <input type="hidden"
				name="steps" id="steps" value="${board.steps}"> <input
			...
			<table border=1>
				...
				<tr>
					<th>메모</th>
					<td><textarea rows="10" cols="20" name="content" id="content">${board.content }</textarea></td>
				</tr>
				<tr>
					<th colspan="2" align="right"><input type="submit"
						value="답글등록" id="reply"> <input type="button" value="수정"
						id="update"> <input type="button" value="삭제" id="delete"
						onclick="del()"></th>
				</tr>
			</table>
			<br>
		...
		<form method="get">
			<input type="hidden" name="b_num" value="${board.b_num }"> <input
				type="hidden" name="id" id="id" value="${sessionLogin}">
			<h5>댓글</h5>
			<%-- <h6 align="right"><a href="myme?bnum=${board.seq }&id=${sessionLogin}">내댓글 보기</a></h6> --%>
